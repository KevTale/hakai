...
async function extractContent(sourceFilename: string) {
  if (!sourceFilename.endsWith('.kai')) {
    throw new Error(`Invalid file extension. Expected .kai file but got: ${sourceFilename}`);
  }
...
function interpolateTemplate(template: string, context: Record<string, TemplateValue>) {
  return template.replace(/\{\{\s*(\w+)\s*\}\}/g, (match, varName) => {
    if (context[varName] === undefined) {
      throw new Error(`Template variable "${varName}" is not defined in context`);
    }
    return String(context[varName]); // Convert value to string for interpolation
  });
}

export async function createHtmlFromTemplate(sourceFilename: string): Promise<string> {
